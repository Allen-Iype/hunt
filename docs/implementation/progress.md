...
| 2026-07-12 | **Discovery internet-wide expansion (Phases A–F): 3 → 13 sources across four tiers.** Tier-2 feeds (RemoteOK, Arbeitnow, We Work Remotely via a dep-free RSS extractor, Hacker News Who's-Hiring; `fetchText`). Tier-3 aggregator APIs (Adzuna, Findwork, JSearch) with **injected** keys resolved from env at the CLI root (`resolveDiscoveryCredentials`); a keyless source registers an **unconfigured stub** → skipped-with-warning, never a crash; `fetchJson` gained an auth-header option. New **`@hunt/eval`** package (9th workspace pkg, `core`-only runtime dep) — discovery-**extraction** scorer (recall × field-accuracy, false-positive rate) + gate; golden snapshots. Tier-4 best-effort web scrapers (LinkedIn, Indeed, Glassdoor) — **honest public HTTP only, no login/credentials, no browser evasion** (SDD §21), each gated by `@hunt/eval` against a golden fragment; block/wall → typed failure pointing at JSearch/paste. Integration: `DiscoveryResult` gained `warnings?` threaded discoverer → `DiscoverJobs` → CLI (`⚠ N sources skipped`); generic `hunt searches add --source <id>:<board>` (validated against `DISCOVERY_ADAPTER_IDS`) reaches every adapter; ATS flags kept. Tests 335 → 397. Validated live: RemoteOK 100 leads + jsearch skipped-with-warning; multi-tier `--source` add + typo rejection. | ADR-0015; SDD §21; internet-wide-job-discovery plan |

---

## Current Focus

Nothing in flight — the **discovery internet-wide expansion (Phases A–F)** is delivered and validated, stopped per milestone workflow for approval. Discovery now spans **13 sources across four tiers** (ATS + feeds + aggregator APIs + best-effort web), each web scraper gated by the new `@hunt/eval` harness; partial-failure runs report skipped sources. Earlier post-V1 progress: M6 (incl. PDF/DOCX) removes the biggest onboarding friction; M7 closes the augment loop; the onboarding arc (seed → confirm → augment) is complete. Next designed candidates: **AI-output eval** (the *other* eval scope — expected extractions + claim-trace pass-rate + fit-score; `@hunt/eval` machinery is ready to host a second scorer) and **analytics + FTS**.
...
- ✅ **Discovery internet-wide expansion (Phases A–F)** — **done**: Tier-2 feeds, Tier-3 aggregator APIs, `@hunt/eval` (discovery-extraction scope), Tier-4 best-effort web scrapers, CLI `--source` + partial-failure warnings. 3 → 13 sources.
- Next designed candidates: **AI-output eval** (a second `@hunt/eval` scorer — expected extractions + claim-trace pass-rate + fit-score; needs live-provider fixtures; the top gap in the 2026-07 reassessment), then **analytics + FTS**.
- Then: browser extension, web UI, interview prep + company research, MCP server, discovery agent.

**Discovery follow-ups:** ~~Lever and Ashby (M9)~~ and ~~aggregator feeds / web tier~~ **done** — discovery spans 13 sources across four tiers. Remaining discovery niceties: a convenience `hunt discover --all` (run all saved searches at once — CLI-only, architecture already supports it); recording *live* golden snapshots for the Tier-4 scrapers to measure real-world recall (the offline eval gate proves parsing correctness on recorded HTML, not live-site availability); and growing the skill dictionary so more discovered leads rank above 0%.
